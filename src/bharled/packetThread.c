...
void exit_routine(){
	printf("\n In error_routine");
	if(pthread_kill(token_depositing_thread, SIGKILL)){
		printf("\n @@@@ ONE");
		if(pthread_kill(server_thread, SIGKILL)){
			printf("\n @@@@ two");
			if(pthread_kill(signal_thread, SIGKILL)){
...
packet *get_packet(int i, long int tokens, long double serv_time){
	...
	if(strcmp(mode, "deterministic") == 0){
		...
		p->service_time = pkt_service_rate;
	}else{
		p->tokens_req = tokens;
		p->service_time = serv_time;
	}
	return p;
}
void extract_pkt_details(char *pkt_details, long double *arr_time, long int *tokens, long double *serv_time){
	...
	//printf("\n ++++++++++++++++ strlen pkt_details: %d", strlen(pkt_details));
...
void arrival_thread_routine(void *file_name){
	...
	if(strcmp(mode, "tracedriven") == 0){
		...
		if(tsfile == NULL){
			fprintf(stderr,"\n *** ERROR : Trace Driven file cannot be opened. Either invalid or corrupt ..... program exiting");
			fclose(tsfile);			
			exit(1);		
		}
		file_is_open = TRUE;
	...
	for(i=1;i<=total_pkts;i++){
		...
		if(p->tokens_req > bucket_capacity){
			printf("\n %Lfms : p%ld arrives, needs %ld tokens, inter arrival time = %Lfms", curr_pkt_time, p->pkt_num, p->tokens_req, curr_pkt_time-prev_pkt_time); 
		...
		printf("\n %Lfms : p%ld arrives, needs %ld tokens, inter arrival time = %Lfms", curr_pkt_time, p->pkt_num, p->tokens_req, iat);
